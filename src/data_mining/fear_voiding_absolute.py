...
def process_csv(df):
    ...
    df["Ear_symmetry"] = np.abs(df["RightEar_x"] - df["LeftEar_x"]) + np.abs(
        df["RightEar_y"] - df["LeftEar_y"]
    )

    df["Tail_elevation"] = (
        df["TailBase_y"] - df["Hipbone_y"]
...
if __name__ == "__main__":
    ...
    output_dir = Path("/home/thomas/washu/behavior_clustering_rewrite/data/data_mined_data/fear_voiding_absolute")
